/**
 * PythonSandboxManager - Python 沙箱管理器
 * 
 * 使用扩展内置的 sandbox.html 作为执行环境
 * 通过 Pyodide 在浏览器中运行 Python 代码
 */
...
class PythonSandboxManager {
    ...
    getSandboxUrl() {
        if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.getURL) {
            return chrome.runtime.getURL('js/runner/languages/python/sandbox.html');
        }
        return null;
    }
                    ...
                    case 'PYTHON_LOADING':
                        ...
                        if (this.pendingOnMessage) {
                            this.pendingOnMessage({
                                level: 'info',
                                data: [data.message || '正在加载 Python 环境...']
                            });
                        }
                        break;
...
if (typeof window !== 'undefined') {
    window.PythonSandboxManager = PythonSandboxManager;
}
